        ...
        require(['plugins/jtopo-0.4.8-min'], function () {
            ...
            // scene.areaSelect = false;
                ...
                function getDatd(falgOne) {
                    if (main.clearInterCharge(_this.interval, 'canvas')) return;
                    $.http.POST('/monitor/getGCTopo.do', {
                ...
                function drawCofMsg() {
                            ...
                            if (4 == b.length && (c = b[1], d = b[2]), this.texts && this.texts.length > 0) {
                                ...
                                for (var i = 0, len = this.texts.length; i < len; i++) {
                                    ...
                                    if (type === 'foldLine') {
                                        ...
                                        a.fillStyle = "rgba(" + this.fontValueColor + ", " + this.alpha + ")";
                                        a.fillText(this.texts[i].value, e - g / 2 + 20, txtAlign);
                                    }
                                }
                    ...
                    $(window).resize(function(){
                        ...
                        var cnCanvas = document.getElementById('canvas');
                    ...
                    function drawFlow() {
                        ...
                        var soNode = addNode($.getI18n('plantMonitor.photovoltaic'), 'photovoltaicIcon', 10*$wid/550, 10, 60, 38);
                        var zcNode = addNode($.getI18n('plantMonitor.battery'), 'storageBoxIcon', 15*$wid/550, 110, 60, 38);
                        var nbqNode = addNode($.getI18n('plantMonitor.inverter'), 'inverterIcon', 220*$wid/550, 72, 64, 64);
                        ...
                        var mccNode = addNode(' ', 'dot11', 320*$wid/550, 70, 3, 3, 'Middle_Right');
                        var dotNode = addNode('', 'dot11', 120*$wid/550, 60, 3, 3);
                        var dotNode1 = addNode('', 'dot11', 120*$wid/550, 80, 3, 3);
                        var bdV, dsV;
                        ...
                        if (beginDrawLink) {
                            ...
                            var pvVal = App.ConvertUnit({ unit:'watt',num:data.pvPower,len:6,split:3});
                            var loadVal = App.ConvertUnit({ unit:'watt',num:data.loadPower,len:6,split:3});

                            addLink({
                                nodeA: bpLineF,
                                nodeZ: bpLineE,
                                addy: -10,
                                noFlowed: pvVal.num === "0.00" ? true : false,
                            });
                            ...
                            addLink({
                                nodeA: pdLineF,
                                nodeZ: pdLineE,
                                noFlowed: gridVal.num === "0.00" && loadVal.num === "0.00" ? true : false,
                                direction: 'horizontal'
                            });
                            //负载
                            addLink({
